...
def test_stl(path):
    ...
    with open(path, 'r') as fin:
        for line in fin.readlines():
            if re.search(r"#([\s]*)include([\s]*)\<([^\>]*)\>", line) is not None:
                head = line.index("<")
                tail = line.rindex(">")
...
def verify_files(dir, content):
    ...
    for filename in os.listdir(dir):
        ...
        if ext.lower() == ".h" or  ext.lower() == ".hpp":
            content.hfiles.append(os.path.join(dir, filename))
            codefiles += 1
        
        elif ext.lower() == ".cpp":
            content.codefiles.append(os.path.join(dir, filename))
            codefiles += 1
...
def main_verify():
    ...
    for file in files_to_include:
        shutil.copy(os.path.join(mydir, file), os.path.join(zipdir, file))
